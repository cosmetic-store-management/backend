...
vi.spyOn(User, 'findById').mockResolvedValue({ email: "test@example.com" });
...
describe("Business Logic: Order Cancellation", () => {
    ...
    it("Không thể hủy đơn hàng đã bắt đầu giao (shipping)", async () => {
        ...
        vi.mocked(orderRepo.findOrderById).mockResolvedValue(order);
    ...
    it("Hủy đơn hàng đã thanh toán (VNPAY) sẽ gọi hàm hoàn tiền", async () => {
        ...
        vi.mocked(orderRepo.findOrderById).mockResolvedValue(order);
        vi.mocked(orderRepo.findOneAndUpdateOrder).mockImplementation(async () => {
            order.orderStatus = "cancelled";
            return order;
        });
        vi.mocked(paymentService.refundPayment).mockResolvedValue(undefined);
        const customer = { _id: order.userId.toString(), role: "customer" };
        await orderService.cancelOrder(order._id.toString(), customer);
