...
describe("authService.register", () => {
    it("tạo user mới thành công khi phone chưa tồn tại", async () => {
        ...
        vi.mocked(authRepo.findOtpByEmail).mockResolvedValue({ isVerified: true });
        vi.mocked(authRepo.deleteOtp).mockResolvedValue({});
        vi.mocked(authRepo.findByPhone).mockResolvedValue(null);
        vi.mocked(authRepo.findByEmail).mockResolvedValue(null);
        vi.mocked(authRepo.create).mockResolvedValue(fakeUser);
        vi.mocked(authRepo.save).mockResolvedValue(undefined);
    ...
    it("throw conflict khi phone đã tồn tại và có password", async () => {
        ...
        vi.mocked(authRepo.findOtpByEmail).mockResolvedValue({ isVerified: true });
        vi.mocked(authRepo.findByPhone).mockResolvedValue(existingUser);
    ...
    it("throw conflict khi email đã tồn tại ở user khác", async () => {
        vi.mocked(authRepo.findOtpByEmail).mockResolvedValue({ isVerified: true });
        vi.mocked(authRepo.findByPhone).mockResolvedValue(null);
        vi.mocked(authRepo.findByEmail).mockResolvedValue(makeFakeUser({ _id: { toString: () => "other_id" } }));
        await expect(authService.register({
            name: "Test",
            phone: "0901111111",
...
describe("OTP Verification", () => {
    ...
    it("verifyOtp thất bại khi mã sai", async () => {
        ...
        await expect(authService.verifyOtp({ email: "test@example.com", otpCode: "000000" })).rejects.toMatchObject({ status: 400 });
    });
});
// ── loginAdmin ────────────────────────────────────────────────────────────────
...
describe("authService.refreshAccessToken", () => {
    it("cấp token mới khi refresh token hợp lệ", async () => {
        ...
        vi.mocked(jwt.verify).mockReturnValue({ id: "user_id_123" });
        vi.mocked(authRepo.findByIdWithRefreshToken).mockResolvedValue(user);
    ...
    it("throw unauthorized khi JWT verify thất bại", async () => {
        ...
        await expect(authService.refreshAccessToken("bad_token")).rejects.toMatchObject({ status: 401 });
    });
    it("throw unauthorized khi refresh token không khớp DB", async () => {
        const user = makeFakeUser({ refreshTokens: ["other_token"] });
        vi.mocked(jwt.verify).mockReturnValue({ id: "user_id_123" });
        vi.mocked(authRepo.findByIdWithRefreshToken).mockResolvedValue(user);
        await expect(authService.refreshAccessToken("valid_refresh_token")).rejects.toMatchObject({ status: 401 });
    });
});
// ── logout ────────────────────────────────────────────────────────────────────
