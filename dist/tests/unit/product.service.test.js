...
vi.mock("../../app/modules/product/dto/product.response.dto.js", () => ({
    mapProduct: (p) => ({ id: p._id?.toString() ?? "pid", name: p.name, slug: p.slug }),
}));
...
describe("productService.getAdminProducts", () => {
    ...
    it("filter theo trạng thái active", async () => {
        vi.mocked(productRepo.findAdmin).mockResolvedValue([makeFakeProduct()]);
...
describe("productService.createProduct", () => {
    ...
    it("tạo sản phẩm thành công với dữ liệu hợp lệ", async () => {
        ...
        vi.mocked(productRepo.findCategoryById).mockResolvedValue({ _id: FAKE_CATEGORY_ID });
    ...
    it("slug được tạo đúng từ tên tiếng Việt", async () => {
        ...
        vi.mocked(productRepo.create).mockResolvedValue(fakeProduct);
        vi.mocked(productRepo.findById).mockResolvedValue(fakeProduct);
        const result = await productService.createProduct({ ...validInput, name: "Kem Dưỡng Da Mặt" });
...
describe("productService.updateProduct", () => {
    it("cập nhật name và slug tự động thay đổi", async () => {
        ...
        vi.mocked(productRepo.findOneBy)
            .mockResolvedValueOnce(fakeProduct) // tìm product
    ...
    it("throw badRequest khi categoryId mới không tồn tại", async () => {
        vi.mocked(productRepo.findOneBy).mockResolvedValueOnce(makeFakeProduct());
        vi.mocked(productRepo.findCategoryById).mockResolvedValue(null);
    ...
    it("update imageUrl mà không đổi slug", async () => {
        ...
        vi.mocked(productRepo.findOneBy).mockResolvedValueOnce(fakeProduct);
        vi.mocked(productRepo.save).mockResolvedValue(undefined);
...
describe("productService.updateProductStatus", () => {
    it("ẩn product (isActive = false) thành công", async () => {
        ...
        vi.mocked(productRepo.findOneBy).mockResolvedValueOnce(fakeProduct);
        vi.mocked(productRepo.save).mockResolvedValue(undefined);
        vi.mocked(productRepo.findById).mockResolvedValue({ ...fakeProduct, isActive: false });
