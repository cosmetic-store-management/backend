...
describe("productService.createProduct", () => {
    ...
    it("tạo sản phẩm thành công với dữ liệu hợp lệ", async () => {
        ...
        vi.mocked(productRepo.create).mockResolvedValue(fakeProduct);
        vi.mocked(productRepo.findById).mockResolvedValue(fakeProduct);
    ...
    it("throw badRequest khi categoryId không tồn tại", async () => {
        ...
        await expect(productService.createProduct(validInput)).rejects.toMatchObject({ status: 400 });
    });
...
describe("productService.deleteProduct", () => {
    it("xóa product và variants thành công", async () => {
        vi.mocked(productRepo.findOneBy).mockResolvedValue(makeFakeProduct());
        vi.mocked(productRepo.findByIdAndDelete).mockResolvedValue(undefined);
    ...
    it("throw notFound khi product không tồn tại", async () => {
        ...
        await expect(productService.deleteProduct("non_existent_id")).rejects.toMatchObject({ status: 404 });
    });
});
// ── updateProduct ─────────────────────────────────────────────────────────────
describe("productService.updateProduct", () => {
    it("cập nhật name và slug tự động thay đổi", async () => {
        ...
        vi.mocked(productRepo.findDocumentById).mockResolvedValueOnce(fakeProduct); // tìm product
        vi.mocked(productRepo.findOneBy).mockResolvedValueOnce(null); // slug mới chưa tồn tại
        vi.mocked(productRepo.save).mockResolvedValue(undefined);
        vi.mocked(productRepo.findById).mockResolvedValue(updatedProduct);
    ...
    it("throw conflict khi slug mới đã tồn tại trong cùng category", async () => {
        ...
        vi.mocked(productRepo.findOneBy)
            .mockResolvedValueOnce(conflictProduct); // slug đã tồn tại
        await expect(productService.updateProduct("product_id", { name: "Kem Dưỡng Ẩm" })).rejects.toMatchObject({ status: 409 });
    });
    it("throw badRequest khi categoryId mới không tồn tại", async () => {
        vi.mocked(productRepo.findDocumentById).mockResolvedValueOnce(makeFakeProduct());
        vi.mocked(productRepo.findCategoryById).mockResolvedValue(null);
        await expect(productService.updateProduct("product_id", { categoryId: "invalid_cat" })).rejects.toMatchObject({ status: 400 });
    });
    it("throw notFound khi product không tồn tại", async () => {
        vi.mocked(productRepo.findDocumentById).mockResolvedValue(null);
        await expect(productService.updateProduct("bad_id", { name: "X" })).rejects.toMatchObject({ status: 404 });
    });
    it("update imageUrl mà không đổi slug", async () => {
        ...
        vi.mocked(productRepo.findDocumentById).mockResolvedValueOnce(fakeProduct);
        vi.mocked(productRepo.save).mockResolvedValue(undefined);
        vi.mocked(productRepo.findById).mockResolvedValue(updated);
...
describe("productService.updateProductStatus", () => {
    it("ẩn product (isActive = false) thành công", async () => {
        ...
        vi.mocked(productRepo.findDocumentBy).mockResolvedValueOnce(fakeProduct);
        vi.mocked(productRepo.save).mockResolvedValue(undefined);
    ...
    it("throw notFound khi product không tồn tại", async () => {
        ...
        await expect(productService.updateProductStatus("bad_id", false)).rejects.toMatchObject({ status: 404 });
    });
});
...
describe("productService.getPublicProductDetail", () => {
    it("trả về product khi slug hợp lệ", async () => {
        ...
        vi.mocked(productRepo.findBySlug).mockResolvedValue(fakeProduct);
    ...
    it("throw notFound khi slug không tồn tại", async () => {
        ...
        await expect(productService.getPublicProductDetail("non-existent")).rejects.toMatchObject({ status: 404 });
    });
});
