...
describe("Cart Service Unit Tests", () => {
    ...
    describe("getCart", () => {
        it("should return empty cart if not found", async () => {
            ...
            vi.mocked(cartRepository.create).mockResolvedValue({ userId: "user1", items: [] });
    ...
    describe("syncCart", () => {
        it("should merge local items into server cart", async () => {
            ...
            vi.mocked(cartRepository.findByUserId)
                .mockResolvedValueOnce(mockServerCart)
            ...
            vi.mocked(Variant.findOne).mockResolvedValue({ _id: "507f1f77bcf86cd799439011", stock: 10, isActive: true, productId: "p1" });
