...
describe("auditLogService.logAction", () => {
    it("gọi repository để tạo log thành công", async () => {
        vi.mocked(auditRepo.createLog).mockResolvedValue({});
        await auditService.logAction("user123", "Admin", "create", "inventory", "Tạo sản phẩm", "192.168.1.1");
    ...
    it("không throw error nếu repository bị lỗi (fire-and-forget)", async () => {
        ...
        await expect(auditService.logAction("user123", "Admin", "create", "inventory", "Desc", "1.1.1.1")).resolves.not.toThrow();
