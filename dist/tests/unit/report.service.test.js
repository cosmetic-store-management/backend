...
beforeEach(() => {
    ...
    vi.mocked(settingService.getSettings).mockResolvedValue({ profitMargin: 35 });
});
describe("reportService.getDashboardStats", () => {
    it("trả về thống kê chính xác với % thay đổi (calcChange)", async () => {
        // Mock current period
        vi.mocked(reportRepo.aggregateRevenue).mockResolvedValue([{ totalRevenue: 1000000, totalCost: 650000 }]);
        vi.mocked(reportRepo.countOrders).mockResolvedValue(10);
        vi.mocked(reportRepo.aggregateSoldProducts).mockResolvedValue([{ totalSold: 50 }]);
        ...
        // Mock top products
        vi.mocked(reportRepo.aggregateTopProducts).mockResolvedValue([{ _id: "prod1", sold: 25 }]);
        vi.mocked(reportRepo.findProductById).mockResolvedValue({ _id: "prod1", name: "Son kem", categoryId: { name: "Son môi" } });
        vi.mocked(reportRepo.findVariantsByProductId).mockResolvedValue([{ stock: 100 }]);
        // Mock low stock
        ...
        // Orders: 5 -> 10 (+100%)
        // Customers: 5 -> 5 (0%)
        vi.mocked(reportRepo.aggregateRevenueForPeriod).mockResolvedValue([{ totalRevenue: 500000 }]);
    ...
    it("trả về revenueChange là 100% nếu kỳ trước là 0 và kỳ này lớn hơn 0", async () => {
        vi.mocked(reportRepo.aggregateRevenue).mockResolvedValue([{ totalRevenue: 1000 }]);
        vi.mocked(reportRepo.countOrders).mockResolvedValue(0);
        vi.mocked(reportRepo.aggregateSoldProducts).mockResolvedValue([]);
        vi.mocked(reportRepo.countCustomers).mockResolvedValue(0);
        vi.mocked(reportRepo.findRecentOrders).mockResolvedValue([]);
