...
vi.mock("../../app/modules/order/dto/order.response.dto.js", () => ({
    mapOrder: (order, items) => ({ ...order, items }),
}));
...
describe("orderService.cancelOrder", () => {
    it("hủy đơn thành công khi đơn ở trạng thái pending", async () => {
        ...
        vi.mocked(orderRepo.findOrderById).mockResolvedValue(fakeOrder);
        vi.mocked(orderRepo.findOneAndUpdateOrder).mockResolvedValue({ ...fakeOrder, orderStatus: "cancelled" });
        vi.mocked(orderRepo.saveOrder).mockResolvedValue(fakeOrder);
        vi.mocked(orderRepo.incrementVariantStock).mockResolvedValue(undefined);
        vi.mocked(User.findById).mockResolvedValue(null);
        const requestUser = {
            _id: { toString: () => FAKE_USER_ID },
            role: "customer",
        };
        const result = await orderService.cancelOrder(FAKE_ORDER_ID, requestUser);
    ...
    it("throw badRequest khi đơn không ở trạng thái pending", async () => {
        ...
        vi.mocked(orderRepo.findOrderById).mockResolvedValue(processingOrder);
        const requestUser = {
            _id: { toString: () => FAKE_USER_ID },
            role: "customer",
        };
        await expect(orderService.cancelOrder(FAKE_ORDER_ID, requestUser)).rejects.toMatchObject({ status: 400 });
    });
    it("throw forbidden khi user không phải chủ đơn hàng và không phải admin", async () => {
        ...
        await expect(orderService.cancelOrder(FAKE_ORDER_ID, requestUser)).rejects.toMatchObject({ status: 403 });
    });
    it("hoàn trả voucher khi hủy đơn có voucher", async () => {
        ...
        await orderService.cancelOrder(FAKE_ORDER_ID, requestUser);
...
describe("orderService.updateOrderStatus", () => {
    it("chuyển trạng thái hợp lệ pending → processing thành công", async () => {
        ...
        vi.mocked(orderRepo.findOne).mockResolvedValue(fakeOrder);
        vi.mocked(orderRepo.findOneAndUpdateOrder).mockResolvedValue({ ...fakeOrder, orderStatus: "processing" });
        vi.mocked(orderRepo.saveOrder).mockResolvedValue(fakeOrder);
        const adminUser = makeAdminUser();
        const result = await orderService.updateOrderStatus(FAKE_ORDER_ID, { orderStatus: "processing" }, adminUser);
    ...
    it("throw badRequest khi chuyển trạng thái không hợp lệ (pending → completed)", async () => {
        ...
        vi.mocked(orderRepo.findOne).mockResolvedValue(fakeOrder);
        const adminUser = makeAdminUser();
        await expect(orderService.updateOrderStatus(FAKE_ORDER_ID, { orderStatus: "completed" }, adminUser)).rejects.toMatchObject({ status: 400 });
    });
});
// ── getOrder ──────────────────────────────────────────────────────────────────
describe("orderService.getOrder", () => {
    it("admin có thể xem bất kỳ đơn hàng nào", async () => {
        ...
        vi.mocked(orderRepo.findOrderById).mockResolvedValue(fakeOrder);
        const adminUser = makeAdminUser();
        const result = await orderService.getOrder(FAKE_ORDER_ID, adminUser);
    ...
    it("customer chỉ xem được đơn hàng của mình", async () => {
        ...
        const result = await orderService.getOrder(FAKE_ORDER_ID, customer);
        expect(result).toBeDefined();
    });
    it("throw forbidden khi customer cố xem đơn của người khác", async () => {
        ...
        await expect(orderService.getOrder(FAKE_ORDER_ID, customer)).rejects.toMatchObject({ status: 403 });
    });
});
// ── requestReturnOrder ───────────────────────────────────────────────────────
describe("orderService.requestReturnOrder", () => {
    it("thành công: đổi trạng thái sang return_pending và lưu lý do", async () => {
        ...
        const result = await orderService.requestReturnOrder(FAKE_ORDER_ID, customer, "Hàng bị lỗi");
    ...
    it("thất bại: throw badRequest nếu không nhập lý do", async () => {
        ...
        await expect(orderService.requestReturnOrder(FAKE_ORDER_ID, customer, "   ")).rejects.toMatchObject({ status: 400 });
    });
...
describe("orderService.updateOrderStatus (Reject Return)", () => {
    it("gửi email từ chối khi chuyển từ return_pending sang completed", async () => {
        ...
        vi.mocked(orderRepo.findOne).mockResolvedValue(fakeOrder);
        vi.mocked(orderRepo.findOneAndUpdateOrder).mockResolvedValue({ ...fakeOrder, orderStatus: "completed" });
        vi.mocked(orderRepo.saveOrder).mockResolvedValue(fakeOrder);
        // Mock user email
        vi.mocked(User.findById).mockReturnValue({
        ...
        const result = await orderService.updateOrderStatus(FAKE_ORDER_ID, { orderStatus: "completed" }, adminUser);
