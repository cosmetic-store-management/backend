...
describe("Flash Sale Service", () => {
    ...
    describe("getActiveFlashSale", () => {
        it("trả về flash sale đang active", async () => {
            ...
            vi.mocked(flashSaleRepo.findActiveFlashSale).mockResolvedValue(fakeFs);
    ...
    describe("createFlashSale", () => {
        it("tạo flash sale thành công", async () => {
            const fakeFs = makeFakeFlashSale();
            vi.mocked(flashSaleRepo.create).mockResolvedValue(fakeFs);
            vi.mocked(flashSaleRepo.findById).mockResolvedValue(fakeFs);
            const payload = {
                name: "Sale mới",
    ...
    describe("updateFlashSale", () => {
        ...
        it("giữ nguyên soldQuantity khi update item cũ", async () => {
            ...
            vi.mocked(flashSaleRepo.findById).mockResolvedValue(fakeFs);
            vi.mocked(flashSaleRepo.update).mockResolvedValue(fakeFs);
