...
describe("[Integration] Auth — register → login flow", () => {
    ...
    it("đăng nhập public thành công sau khi đăng ký", async () => {
        ...
        const result = await authService.loginPublic({ phone: "0901111111", password: "StrongPass@123" });
    ...
    it("đăng ký hai lần với cùng phone sẽ throw conflict", async () => {
        await authService.register(newUser);
        await expect(authService.register(newUser)).rejects.toMatchObject({ status: 409 });
    });
});
// ── refresh token rotation ────────────────────────────────────────────────────
describe("[Integration] Auth — refresh token rotation", () => {
    it("refresh token rotation cấp cặp token mới và invalidate token cũ", async () => {
        const { refreshToken: oldRefreshToken } = await authService.register({
            name: "User A", phone: "0902222222", email: "usera@example.com", password: "Pass@123",
        });
        // JWT iat có độ phân giải 1 giây — đợi để token mới có iat khác
        await new Promise(r => setTimeout(r, 1100));
        const { accessToken: newAccess, refreshToken: newRefresh } = await authService.refreshAccessToken(oldRefreshToken);
        expect(newAccess).toBeTruthy();
        expect(newRefresh).not.toBe(oldRefreshToken); // token đã xoay
        // Old refresh token phải bị reject
...
describe("[Integration] Auth — changePassword", () => {
    it("đổi mật khẩu thành công, đăng nhập lại với mật khẩu mới", async () => {
        const { user } = await authService.register({
            name: "User B", phone: "0903333333", email: "userb@example.com", password: "OldPass@123",
        });
        ...
        const result = await authService.loginPublic({ phone: "0903333333", password: "NewPass@456" });
        expect(result.user.id).toBe(user.id);
        // Đăng nhập với mật khẩu cũ phải thất bại
        await expect(authService.loginPublic({ phone: "0903333333", password: "OldPass@123" }))
            .rejects.toMatchObject({ status: 401 });
    });
});
