...
const seedOrder = async (overrides = {}) => Order.create({
    code: "ORD-IT-001",
...
describe("[Integration] Order — updateOrderStatus (staff)", () => {
    it("chuyển pending → processing thành công", async () => {
        const order = await seedOrder();
        const result = await orderService.updateOrderStatus(order._id.toString(), { orderStatus: "processing" }, staffUser);
    ...
    it("throw badRequest khi chuyển pending → completed (bỏ qua bước)", async () => {
        const order = await seedOrder();
        await expect(orderService.updateOrderStatus(order._id.toString(), { orderStatus: "completed" }, staffUser)).rejects.toMatchObject({ status: 400 });
    });
});
// ── getMyOrders ───────────────────────────────────────────────────────────────
