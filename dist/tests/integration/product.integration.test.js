...
describe("[Integration] Product — updateProduct", () => {
    it("cập nhật tên và slug thay đổi tương ứng", async () => {
        ...
        const updated = await productService.updateProduct(created.id, { name: "Kem Mới" });
    ...
    it("throw conflict khi tên mới trùng slug với product khác trong cùng category", async () => {
        await productService.createProduct(makeProductInput({ name: "Serum X", variants: [{ name: "30ml", sku: "SKU-SX", price: 100_000, stock: 10 }] }));
        const p2 = await productService.createProduct(makeProductInput({ name: "Toner Y", variants: [{ name: "150ml", sku: "SKU-TY", price: 90_000, stock: 20 }] }));
        // Đổi tên Toner Y → Serum X (slug trùng)
        await expect(productService.updateProduct(p2.id, { name: "Serum X" }))
            .rejects.toMatchObject({ status: 409 });
    });
