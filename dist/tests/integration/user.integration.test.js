...
// ── Helpers ───────────────────────────────────────────────────────────────────
const createCustomer = (overrides = {}) => User.create({ name: "Khách Test", phone: "0900000001", role: "customer", isActive: true, points: 100, ...overrides });
const createOwner = () => User.create({ name: "Owner", phone: "0999999999", role: "owner", isActive: true, points: 0 });
const createStaffUser = (phone = "0911111111") => User.create({ name: "Staff Test", phone, role: "staff", isActive: true, points: 0 });
// ── Profile ───────────────────────────────────────────────────────────────────
...
describe("[Integration] User — createStaff", () => {
    ...
    it("manager tạo staff nhưng không thể tạo manager (bị ép về staff)", async () => {
        const manager = await User.create({ name: "Mgr", phone: "0933333333", role: "manager", isActive: true });
        const result = await userService.createStaff({
