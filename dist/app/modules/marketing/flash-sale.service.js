...
export const getAllFlashSales = async (filters = {}, page = 1, limit = 10) => {
    const query = {};
    const now = new Date();
    // Status Filter
...
export const getFlashSaleById = async (id) => {
    ...
    if (!fs)
        throw notFound("Flash Sale does not exist");
    return mapFlashSale(fs);
};
import FlashSale from "./models/flash-sale.schema.js";
export const validateFlashSaleItems = async (items, startTime, endTime, excludeId) => {
    ...
    for (const item of items) {
        ...
        if (item.flashPrice >= normalPrice) {
            throw badRequest(`Flash price (${item.flashPrice} ₫) must be less than normal price (${normalPrice} ₫) for ${product.name} (${variant.name})`);
        }
    }
    const overlapQuery = {
        isActive: true,
        ...
        for (const event of overlappingEvents) {
            for (const eventItem of event.items) {
                if (activeVariantIds.has(eventItem.variantId.toString())) {
                    ...
                    throw badRequest(`Variant ${matchedVariant?.name || eventItem.variantId} already belongs to another overlapping active/upcoming flash sale event: "${event.name}"`);
                }
            }
        }
    }
