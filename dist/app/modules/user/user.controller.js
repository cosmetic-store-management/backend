...
import { authenticate, isManager, isAuthenticated, requirePermission, authorize, } from "../../middlewares/auth.middleware.js";
...
router.get("/", authenticate, isManager, catchAsync(async (req, res) => {
    ...
    const hiringStatus = req.query.hiringStatus;
    const workingShift = req.query.workingShift;
    const result = await userService.getStaffUsers(page, limit, search, status, role, hiringStatus, workingShift);
...
router.get("/me/tier-info", authenticate, isAuthenticated, catchAsync(async (req, res) => {
    ...
    return response.success(res, tierInfo);
}));
// PATCH /api/users/me
router.patch("/me", authenticate, isAuthenticated, validate(UpdateProfileSchema), catchAsync(async (req, res) => {
    ...
    await logAction(req.user._id.toString(), req.user.name, "update", "identity", `Updated personal profile`, req.ip || "127.0.0.1");
...
router.patch("/me/avatar", authenticate, isAuthenticated, catchAsync(async (req, res) => {
    ...
    if (!avatar)
        throw { status: 400, message: "Missing image data" };
...
router.post("/me/addresses", authenticate, isAuthenticated, validate(AddressSchema), catchAsync(async (req, res) => {
    ...
    await logAction(req.user._id.toString(), req.user.name, "update", "identity", `Added a new address`, req.ip || "127.0.0.1");
...
router.put("/me/addresses/:addressId", authenticate, isAuthenticated, validate(AddressSchema), catchAsync(async (req, res) => {
    ...
    await logAction(req.user._id.toString(), req.user.name, "update", "identity", `Updated address`, req.ip || "127.0.0.1");
    return response.success(res, {
...
router.delete("/me/addresses/:addressId", authenticate, isAuthenticated, catchAsync(async (req, res) => {
    const user = await userService.deleteAddress(req.user._id.toString(), req.params.addressId);
    await logAction(req.user._id.toString(), req.user.name, "delete", "identity", `Deleted address`, req.ip || "127.0.0.1");
...
router.post("/me/viewed/:productId", authenticate, isAuthenticated, catchAsync(async (req, res) => {
    await userService.recordRecentlyViewed(req.user._id.toString(), req.params.productId);
...
router.delete("/me/viewed/:productId", authenticate, isAuthenticated, catchAsync(async (req, res) => {
    await userService.removeFromViewed(req.user._id.toString(), req.params.productId);
...
router.get("/customers", authenticate, requirePermission("customers.view"), catchAsync(async (req, res) => {
    ...
    const status = req.query.status;
    const spending = req.query.spending;
    const lastPurchase = req.query.lastPurchase;
...
router.post("/customers", authenticate, requirePermission("customers.manage"), catchAsync(async (req, res) => {
    ...
    await logAction(req.user._id.toString(), req.user.name, "create", "identity", `Created customer member account "${customer.name}"`, req.ip || "127.0.0.1");
...
router.post("/staff", authenticate, isManager, validate(CreateStaffSchema), catchAsync(async (req, res) => {
    ...
    await logAction(req.user._id.toString(), req.user.name, "create", "identity", `Created staff account "${staff.name}"`, req.ip || "127.0.0.1");
    return response.created(res, {
...
router.patch("/:id", authenticate, isManager, catchAsync(async (req, res) => {
    const user = await userService.updateUserByAdmin(req.params.id, req.body, req.user);
    await logAction(req.user._id.toString(), req.user.name, "update", "identity", `Edited account "${user.name}" (${user.role})`, req.ip || "127.0.0.1");
    return response.success(res, { user });
}));
...
router.patch("/:id/staff-notes", authenticate, isManager, catchAsync(async (req, res) => {
    const { internalNotes } = req.body;
    const user = await userService.updateStaffInternalNotes(req.params.id, internalNotes, req.user);
    return response.success(res, { user });
}));
...
router.patch("/:id/points", authenticate, isManager, catchAsync(async (req, res) => {
    ...
    const user = await userService.adjustUserPoints(req.params.id, parseInt(pointsChanged, 10), reason, req.user._id.toString());
    return response.success(res, { user });
}));
// DELETE /api/users/:id - admin only, requires users.delete
router.delete("/:id", authenticate, authorize("owner", "manager"), requirePermission("users.delete"), catchAsync(async (req, res) => {
    await userService.deleteUser(req.params.id, req.user);
    await logAction(req.user._id.toString(), req.user.name, "delete", "identity", `Xóa tài khoản nhân viên ${req.params.id}`, req.ip || "127.0.0.1");
