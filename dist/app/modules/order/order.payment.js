...
export const createVnpayUrl = async (orderId, requestUser, ipAddr) => {
...
export const handleVnpayIpn = async (query) => {
    ...
    if (order.totalAmount * 100 !== Number(query.vnp_Amount))
    ...
    const updatedOrder = await Order.findOneAndUpdate({ _id: order._id, paymentStatus: "pending" }, { $set: { paymentStatus: targetStatus } }, { returnDocument: "after" });
    if (!updatedOrder) {
