...
import { calculateTierDiscount, generateOrderCode, getOrderSettings, } from "./checkout.helper.js";
...
export const previewOrder = async (user, data) => {
    ...
    const [variantsList, activeFlashSale] = await Promise.all([
        orderRepo.findVariantsByIds(variantIds),
        findActiveFlashSale(),
    ]);
    ...
    for (const item of data.items) {
        ...
        let unitPrice = variant.discountPrice && variant.discountPrice > 0
            ...
            : variant.price;
        if (activeFlashSale) {
            const fsItem = activeFlashSale.items.find((fsItem) => fsItem.variantId._id.toString() === item.variantId);
    ...
    const shippingFee = await calcShippingFeeFromSettings(subtotal, totalItems, data.province || "", data.district || "", data.ward || "", data.street || "", data.channel || "online");
    // Khách hàng cho POS
    ...
    const userTotalSpent = customerUser?.role === "customer" && customerUser._id
        ...
        : 0;
    const tierDiscountAmount = customerUser?.role === "customer"
    ...
    const productDiscount = Math.min(tierDiscountAmount + voucherDiscountAmount, subtotal);
    ...
    const requestedUsedPoints = typeof data.usedPoints === "number" && data.usedPoints > 0
        ? data.usedPoints
        : 0;
    const maxPointsAllowed = Math.floor(totalAmountBeforePoints * orderSettings.maxPointsPct);
...
export const createOrder = async (user, data) => {
    ...
    const [productsList, variantsList, activeFlashSale] = await Promise.all([
        orderRepo.findProductsByIds(productIds),
        orderRepo.findVariantsByIds(variantIds),
        findActiveFlashSale(),
    ]);
    ...
    const shippingFee = await calcShippingFeeFromSettings(subtotal, totalItems, province, district, ward, street, "online");
    ...
    const tierDiscountAmount = user.role === "customer"
        ? calculateTierDiscount(userTotalSpent, subtotal)
        : 0;
    ...
    try {
        ...
        const sortedItemsToDeduct = [...normalizedItems].sort((a, b) => a.variantId.toString().localeCompare(b.variantId.toString()));
        for (const item of sortedItemsToDeduct) {
            await orderRepo.decrementVariantStock(item.variantId.toString(), item.quantity, session);
            const costPriceTotal = await inventoryRepo.deductBatchesFIFO(item.variantId, item.quantity, session);
        ...
        if (activeFlashSale) {
            for (const item of normalizedItems) {
                const fsItem = activeFlashSale.items.find((fsItem) => fsItem.variantId._id.toString() === item.variantId);
                if (fsItem && fsItem.soldQuantity + item.quantity <= fsItem.quantityLimit) {
                    await incrementFlashSaleSoldQuantity(activeFlashSale._id.toString(), item.variantId, item.quantity, session);
                }
            }
        }
        // Trừ điểm của khách nếu dùng
        if (actualUsedPoints > 0) {
            const updatedUser = await User.findOneAndUpdate({ _id: user._id, points: { $gte: actualUsedPoints } }, { $inc: { points: -actualUsedPoints } }, { session, returnDocument: "after" });
            if (!updatedUser) {
        ...
        if (error.code === 11000 && data.idempotencyKey) {
            ...
            if (existingOrder) {
                return mapOrder(existingOrder, existingOrder.items || []);
            }
        }
        throw badRequest(error.message || "Lỗi trong quá trình tạo đơn hàng, vui lòng thử lại.");
    }
    finally {
...
export const createPOSOrder = async (operator, data) => {
    ...
    try {
        ...
        if (customerUser) {
            ...
            const updatedUser = await User.findOneAndUpdate(updateQuery, { $inc: { points: netPoints } }, { session, returnDocument: "after" });
            if (!updatedUser) {
