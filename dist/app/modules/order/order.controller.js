...
import { authenticate, isStaff, isAuthenticated } from "../../middlewares/auth.middleware.js";
...
router.post("/:id/create-payment-url", authenticate, catchAsync(async (req, res) => {
    ...
    const result = await orderService.createVnpayUrl(req.params.id, req.user, ipAddr);
    return response.success(res, result);
}));
