...
export const calcShippingFeeFromSettings = async (_subtotal, _totalItems, province, district, _ward, _street, channel) => {
    if (channel === "pos")
        return 0;
    const settings = (await getSettings());
    const storeAddress = (settings?.storeAddress || "Hà Nội, Việt Nam").toLowerCase();
    ...
    const isHCMSynonyms = (p) => p === "hcm" ||
        p.includes("ho chi minh") ||
        p === "sg" ||
        p.includes("sai gon");
    const isHNSynonyms = (p) => p === "hn" || p.includes("ha noi");
    ...
    const hasHuyenKeyword = rawDist.includes("huyện") ||
