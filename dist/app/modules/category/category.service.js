...
const getRecursiveCountsMap = async () => {
    ...
    const directCountsMap = await categoryRepo.countProductsByCategoryIds(allCategories.map((c) => c._id));
    const categoryMap = new Map();
...
export const getAdminCategories = async ({ search, status, page = 1, limit = 20, }) => {
    ...
    return {
        categories: categories.map((cat) => {
            cat.productCount = countsMap.get(cat._id.toString())?.productCount || 0;
            return mapCategory(cat);
        }),
        pagination: {
            limit: parsedLimit,
