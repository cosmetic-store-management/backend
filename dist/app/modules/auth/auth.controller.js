...
import { RegisterSchema, LoginSchema, ChangePasswordSchema, ForgotPasswordSchema, ResetPasswordSchema, SendOtpSchema, VerifyOtpSchema, } from "./dto/auth.request.dto.js";
...
router.get("/google/callback", passport.authenticate("google", { session: false, failureRedirect: `${FRONTEND_URL}/login?error=oauth_failed` }), catchAsync(async (req, res) => {
    if (!req.user)
        return res.redirect(`${FRONTEND_URL}/login?error=oauth_failed`);
    // req.user được gán bởi Strategy (đã check/create trong DB)
    const tokens = await authService.generateTokensForOAuth(req.user);
    res.redirect(`${FRONTEND_URL}/auth/social-callback?token=${tokens.accessToken}&refreshToken=${tokens.refreshToken}`);
}));
// Facebook
router.get("/facebook", passport.authenticate("facebook", { scope: ["email"] }));
router.get("/facebook/callback", passport.authenticate("facebook", { session: false, failureRedirect: `${FRONTEND_URL}/login?error=oauth_failed` }), catchAsync(async (req, res) => {
    if (!req.user)
        return res.redirect(`${FRONTEND_URL}/login?error=oauth_failed`);
    const tokens = await authService.generateTokensForOAuth(req.user);
    res.redirect(`${FRONTEND_URL}/auth/social-callback?token=${tokens.accessToken}&refreshToken=${tokens.refreshToken}`);
...
router.post("/public/send-otp", authLimiter, validate(SendOtpSchema), catchAsync(async (req, res) => {
    const result = await authService.sendOtp(req.body);
    return response.success(res, result);
}));
// POST /api/auth/verify-otp
router.post("/public/verify-otp", authLimiter, validate(VerifyOtpSchema), catchAsync(async (req, res) => {
