...
export const findPublicByIds = async (ids) => {
    ...
    const sorted = ids.map(id => productMap.get(id.toString())).filter(Boolean);
    return attachVariants(sorted);
};
export const findAdmin = async (query, skip, limit) => {
    const products = await Product.find(query)
...
export const findOneBy = (query) => Product.findOne(query);
