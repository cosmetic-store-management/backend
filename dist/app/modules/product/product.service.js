...
export const getPublicProducts = async ({ category, brandId, search, onSale, page = 1, limit = 12, minPrice, maxPrice, brands, sort }) => {
    ...
    if (onSale === "true") {
        ...
        const saleProductIds = [...new Set(saleVariants.map((v) => v.productId.toString()))];
        query._id = { $in: saleProductIds };
    }
    if (minPrice !== undefined || maxPrice !== undefined) {
        const variantQuery = {};
        if (minPrice !== undefined)
            variantQuery.$gte = Number(minPrice);
        if (maxPrice !== undefined)
            variantQuery.$lte = Number(maxPrice);
    ...
    if (category) {
        ...
        if (categoryIds.length === 0)
            return { products: [], pagination: { page: parsedPage, limit: parsedLimit, total: 0, totalPages: 0 }, availableBrands: [], availableCategoryIds: [] };
        query.$or = [
    ...
    try {
        ...
        const [availableBrands, availableCategoryIds] = await Promise.all([
            productRepo.findBrandsInProducts(queryWithoutBrands).catch((e) => { console.error('[findBrands]', e.message); return []; }),
            productRepo.findCategoriesInProducts(queryWithoutCategories).catch((e) => { console.error('[findCats]', e.message); return []; }),
        ]);
        ...
        if (isPrice) {
            ...
            const pagedIds = priceSorted.slice(skip, skip + parsedLimit).map((r) => r._id);
...
export const getAdminProducts = async ({ search, category, brandId, status, page = 1, limit = 20, shopId }) => {
    const parsedPage = Math.max(Number(page) || 1, 1);
    ...
    if (category) {
        ...
        if (categoryIds.length === 0)
            return { products: [], pagination: { page: parsedPage, limit: parsedLimit, total: 0, totalPages: 0 } };
        query.$or = [
            { categoryId: { $in: categoryIds } },
            { categoryIds: { $in: categoryIds } },
        ];
