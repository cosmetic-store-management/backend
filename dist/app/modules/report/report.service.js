...
export const getDashboardStats = async (startDate, endDate) => {
    ...
    const [prevRevenueResult, prevOrdersCount, prevCustomersCount,] = await Promise.all([
    ...
    const recentOrders = recentOrdersRaw.map((o) => {
        ...
        if (diffHours > 0)
            dateStr = `${diffHours} giờ trước`;
        else if (diffMins > 0)
            dateStr = `${diffMins} phút trước`;
        const statusMap = {
    ...
    for (const agg of topProductsAgg) {
        ...
        if (prod) {
            ...
            topProducts.push({
                name: prod.name,
                category: prod.categoryId?.name ?? "Mỹ phẩm",
                sold: agg.sold,
                stock: totalStock,
    ...
    // ── Low Stock ──────────────────────────────────────────────────────────────
    const lowStockItems = await Promise.all(lowStockRaw.map(async (v) => {
        const prod = await reportRepo.findProductById(v.productId);
        return {
...
// ── Completion Rates ──────────────────────────────────────────────────────────
export const getCompletionRates = async (startDate, endDate) => {
    const dateFilter = buildDateFilter(startDate, endDate);
...
// ── Payment Methods ───────────────────────────────────────────────────────────
export const getPaymentMethodsStats = async (startDate, endDate) => {
    const dateFilter = buildDateFilter(startDate, endDate);
...
// ── Voucher Stats ─────────────────────────────────────────────────────────────
export const getVoucherStats = async (startDate, endDate) => {
    const dateFilter = buildDateFilter(startDate, endDate);
