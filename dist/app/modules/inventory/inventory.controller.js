...
router.get("/stock", authenticate, isStaff, catchAsync(async (req, res) => {
    ...
    const result = await inventoryService.getStockList(search, Number(page), Number(limit));
...
router.post("/stock/adjust", authenticate, isStaff, validate(AdjustStockSchema), catchAsync(async (req, res) => {
    ...
    await logAction(req.user._id.toString(), req.user.name, "update", "inventory", `Kiểm kho sản phẩm "${variant.name}": tồn kho thực tế ${req.body.actualStock}`, req.ip || "127.0.0.1");
