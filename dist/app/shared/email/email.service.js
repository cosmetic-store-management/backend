...
const getFromEmail = () => process.env.EMAIL_FROM || "GlowUp Cosmetics <onboarding@resend.dev>";
...
async function sendEmailWithRetry(payload, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.error(`❌ Lỗi gửi email (lần ${attempt}/${maxRetries}) [${payload.subject}]:`, err.message);
            if (attempt === maxRetries) {
                console.error(`🚨 Thất bại gửi email sau ${maxRetries} lần thử tới:`, payload.to);
                return;
            }
...
// ── 2. OTP Verification ────────────────────────────────────────────────────────
export const sendOtpVerificationEmail = async (to, otpCode) => {
    const body = `
...
// ── 2. Order Confirmed ────────────────────────────────────────────────────────
export const sendOrderSuccessEmail = async (to, orderCode, totalAmount) => {
...
// ── 3. Order Shipped ──────────────────────────────────────────────────────────
export const sendOrderShippedEmail = async (to, orderCode, trackingCode) => {
    const orderLink = `${FRONTEND_URL()}/account/orders`;
...
// ── 4. Order Cancelled ────────────────────────────────────────────────────────
export const sendOrderCancelledEmail = async (to, orderCode, isPaid = false) => {
...
// ── 5. Order Returned ────────────────────────────────────────────────────────
export const sendOrderReturnedEmail = async (to, orderCode) => {
    const shopLink = `${FRONTEND_URL()}/`;
    const body = `
...
export const sendOrderReturnRejectedEmail = async (to, orderCode) => {
    const shopLink = `${FRONTEND_URL()}/`;
    const body = `
...
// ── 7. Order Completed ──────────────────────────────────────────────────────────
export const sendOrderCompletedEmail = async (to, orderCode) => {
    const shopLink = `${FRONTEND_URL()}/`;
    const body = `
    <div style="background:#f0fdf4;border-left:4px solid #22c55e;border-radius:4px;padding:14px 18px;margin-bottom:24px;">
