...
vi.mock("../../app/modules/voucher/dto/voucher.response.dto.js", () => ({
  mapVoucher: (v: any) => ({ id: v._id?.toString() ?? "vid", code: v.code, discountType: v.discountType }),
}));
...
describe("voucherService.validateVoucher", () => {
  it("trả về discountAmount đúng với voucher percent", async () => {
    vi.mocked(voucherRepo.findByCode).mockResolvedValue(makeFakeVoucher() as any);
  ...
  it("throw badRequest khi voucher bị vô hiệu hóa", async () => {
    vi.mocked(voucherRepo.findByCode).mockResolvedValue(makeFakeVoucher({ isActive: false }) as any);
  ...
  it("throw badRequest khi đơn hàng dưới minOrderValue", async () => {
    vi.mocked(voucherRepo.findByCode).mockResolvedValue(makeFakeVoucher({ minOrderValue: 500_000 }) as any);
...
describe("voucherService.createVoucher", () => {
  ...
  it("tạo voucher thành công khi code chưa tồn tại", async () => {
    ...
    vi.mocked(voucherRepo.create).mockResolvedValue(makeFakeVoucher({ code: "NEW10" }) as any);
  ...
  it("throw conflict khi code đã tồn tại", async () => {
    vi.mocked(voucherRepo.findByCodeExact).mockResolvedValue(makeFakeVoucher() as any);

    await expect(voucherService.createVoucher(validInput as any))
  ...
  it("throw badRequest khi startDate >= endDate", async () => {
    ...
    await expect(voucherService.createVoucher(badInput as any))
      .rejects.toMatchObject({ status: 400 });
  });
});
