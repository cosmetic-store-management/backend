...
vi.mock("../../app/models/product.schema.js", () => ({
  default: { findById: vi.fn(), findByIdAndUpdate: vi.fn().mockResolvedValue(null) },
}));
...
describe("reviewService.createReview", () => {
  ...
  it("tạo review thành công khi user đã mua và chưa review", async () => {
    vi.mocked(Product.findById as any).mockReturnValue({ lean: () => Promise.resolve({ _id: FAKE_PRODUCT_ID }) });
    vi.mocked(reviewRepo.findOne).mockResolvedValue(null);
    vi.mocked(orderRepo.findOrdersByUserId).mockResolvedValue([
      { orderStatus: "completed", items: [{ productId: { toString: () => FAKE_PRODUCT_ID } }] } as any,
    ]);
    vi.mocked(reviewRepo.create).mockResolvedValue(makeFakeReview() as any);
    vi.mocked(reviewRepo.aggregateStats).mockResolvedValue([{ averageRating: 5, totalReviews: 1 }] as any);
    vi.mocked(reviewRepo.updateProductStats).mockResolvedValue(null as any);

    const result = await reviewService.createReview(FAKE_USER_ID, input);
  ...
  it("throw forbidden khi user chưa mua sản phẩm", async () => {
    ...
    await expect(reviewService.createReview(FAKE_USER_ID, input))
  ...
  it("throw badRequest khi productId không hợp lệ", async () => {
    await expect(reviewService.createReview(FAKE_USER_ID, { ...input, productId: "invalid_id" }))
      .rejects.toMatchObject({ status: 400 });
  });
});
...
describe("reviewService.deleteReviewByUser", () => {
  it("xóa thành công khi review thuộc về user", async () => {
    vi.mocked(reviewRepo.findOneAndDelete).mockResolvedValue(makeFakeReview() as any);
  ...
  it("throw forbidden khi review không thuộc về user", async () => {
    ...
    await expect(reviewService.deleteReviewByUser(FAKE_USER_ID, FAKE_REVIEW_ID))
      .rejects.toMatchObject({ status: 403 });
  });
});
...
describe("reviewService.updateReviewByUser", () => {
  it("cập nhật rating thành công", async () => {
    ...
    vi.mocked(reviewRepo.aggregateStats).mockResolvedValue([{ averageRating: 4, totalReviews: 1 }] as any);
    vi.mocked(reviewRepo.updateProductStats).mockResolvedValue(null as any);
  ...
  it("throw forbidden khi review không thuộc về user", async () => {
    ...
    await expect(reviewService.updateReviewByUser(FAKE_USER_ID, FAKE_REVIEW_ID, 5))
      .rejects.toMatchObject({ status: 403 });
  });
});
