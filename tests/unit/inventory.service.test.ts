...
describe("inventoryService.createGoodsReceipt", () => {
  ...
  it("cộng stock và tạo transaction sau khi nhập kho thành công", async () => {
    ...
    vi.mocked(inventoryRepo.findSupplierById).mockResolvedValue(
      fakeSupplier as any,
    );
    vi.mocked(inventoryRepo.findVariantById).mockResolvedValue(
      fakeVariant as any,
    );
    vi.mocked(inventoryRepo.findProductById).mockResolvedValue(
      fakeProduct as any,
    );
    ...
    vi.mocked(inventoryRepo.atomicUpdateStock as any).mockResolvedValue({
      ...fakeVariant,
      stock: 50,
    });
    ...
    expect(inventoryRepo.atomicUpdateStock).toHaveBeenCalledWith(
      fakeVariant._id,
      20,
      expect.anything()
    );
  ...
  it("throw badRequest khi items rỗng", async () => {
    await expect(
      inventoryService.createGoodsReceipt(fakeOperator as any, {
        supplierId: FAKE_SUPPLIER_ID,
        items: [],
      }),
...
describe("inventoryService.adjustStock", () => {
  ...
  it("throw notFound khi variant không tồn tại", async () => {
    ...
    await expect(
      inventoryService.adjustStock(fakeOperator as any, {
        variantId: FAKE_VARIANT_ID,
        actualStock: 10,
      }),
    ).rejects.toMatchObject({ status: 404 });
...
describe("inventoryService.createSupplier", () => {
  it("tạo nhà cung cấp thành công với name và phone hợp lệ", async () => {
    ...
    vi.mocked(inventoryRepo.createSupplier).mockResolvedValue(
      fakeSupplier as any,
    );
  ...
  it("throw badRequest khi thiếu name hoặc phone", async () => {
    await expect(
      inventoryService.createSupplier({ name: "", phone: "" }),
    ).rejects.toMatchObject({ status: 400 });
  });
});
