...
const getBrandProductCounts = async (brandIds: string[]): Promise<Record<string, number>> => {
...
interface AdminBrandQuery { search?: string; status?: string; page?: number; limit?: number; }

export const getAdminBrands = async ({ search, status, page = 1, limit = 50 }: AdminBrandQuery) => {
