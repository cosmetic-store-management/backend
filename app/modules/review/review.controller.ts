...
import { authenticate, isManager, isStaff } from "../../middlewares/auth.middleware.js";
...
router.patch("/admin/:id/reply", authenticate, isStaff, catchAsync(async (req, res) => {
  const reviewId = req.params.id as string;
  ...
  return response.success(res, { message: "Đã phản hồi đánh giá", review: result });
}));
...
router.patch("/:id", authenticate, catchAsync(async (req, res) => {
  ...
  const result = await reviewService.updateReviewByUser(req.user!._id.toString(), req.params.id as string, rating, comment, images);
  return response.success(res, { message: "Cập nhật đánh giá thành công", review: result });
}));

router.delete("/:id", authenticate, catchAsync(async (req, res) => {
  const { id } = req.params;
  await reviewService.deleteReviewByUser(req.user!._id.toString(), id as string);
...
router.post("/", authenticate, validate(CreateReviewSchema), catchAsync(async (req, res) => {
  ...
  return response.created(res, { message: "Đánh giá sản phẩm thành công", review });
}));
