...
import type { CreateVoucherInput, UpdateVoucherInput } from "./dto/voucher.request.dto.js";
...
export const validateVoucher = async (code: string, subtotal: number, shippingFee = 30000, userId?: string) => {
  ...
  const now = new Date();
  if (now < voucher.startDate) throw badRequest("Mã giảm giá chưa đến thời gian sử dụng");
 ...
 */
export const incrementVoucherUsage = (code: string, userId?: string) =>
  voucherRepo.atomicIncrementUsage(code, userId);

/**
 * Atomic decrement usedCount — dùng khi order bị cancel/thất bại (rollback).
 * Guard: chỉ decrement nếu usedCount > 0.
 */
export const decrementVoucherUsage = (code: string, userId?: string) =>
  voucherRepo.atomicDecrementUsage(code, userId);
