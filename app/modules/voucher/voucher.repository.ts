...
export const findByCodeExact = (code: string) =>
  Voucher.findOne({ code });

export const create = (data: any) =>
  Voucher.create(data);

export const save = (voucher: any) =>
  voucher.save();
 ...
 */
export const atomicIncrementUsage = async (code: string, userId?: string) => {
  const update: any = { $inc: { usedCount: 1 } };
  if (userId) update.$addToSet = { usedBy: new mongoose.Types.ObjectId(userId) };
 ...
 */
export const atomicDecrementUsage = (code: string, userId?: string) => {
  const update: any = { $inc: { usedCount: -1 } };
...
export const addVoucherToWallet = (userId: string, voucherId: mongoose.Types.ObjectId) =>
...
export const removeVoucherFromWallet = (userId: string, voucherId: mongoose.Types.ObjectId) =>
  User.findByIdAndUpdate(
    userId,
