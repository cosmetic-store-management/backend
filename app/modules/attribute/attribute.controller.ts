...
router.get(
  ...
  catchAsync(async (req, res) => {
    const attribute = await attrService.getAttributeDetail(
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    await logAction(
      ...
      "create",
      "catalog",
      `Tạo thuộc tính biến thể "${attribute.name}" (Mã: ${attribute.code})`,
      req.ip || "127.0.0.1",
    );
...
router.patch(
  ...
  catchAsync(async (req, res) => {
    const attribute = await attrService.updateAttribute(
      req.params.id as string,
      req.body,
    );
    await logAction(
      ...
      "update",
      "catalog",
      `Cập nhật thuộc tính biến thể "${attribute.name}"`,
      req.ip || "127.0.0.1",
    );
  ...
  catchAsync(async (req, res) => {
    ...
    if (attribute) {
      await logAction(
        ...
        "delete",
        "catalog",
        `Xóa thuộc tính biến thể "${attribute.name}"`,
        req.ip || "127.0.0.1",
      );
    }
