...
const mapCategoryRef = (
  raw: Types.ObjectId | PopulatedCategory | null | undefined,
): CategoryRef | null => {
  if (!raw) return null;
...
const mapBrandRef = (
  raw: Types.ObjectId | PopulatedBrand | null | undefined,
): BrandRef | null => {
  if (!raw) return null;
  if (typeof raw === "object" && "name" in raw) {
    return {
...
export const mapProduct = (
  product: ProductWithPopulated | ProductDocument,
): ProductResponse => {
  ...
  return {
    ...
    slug: product.slug,
    brandId: p.brandId
      ? typeof p.brandId === "object" && "_id" in (p.brandId as any)
