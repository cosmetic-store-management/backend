import Product, { type ProductDocument, type IProduct } from "../../models/product.schema.js";
...
export function attachVariants(products: ProductDocument[]): Promise<ProductDocument[]>;
export function attachVariants(products: ProductDocument): Promise<ProductDocument>;
export async function attachVariants(products: ProductDocument | ProductDocument[]): Promise<ProductDocument | ProductDocument[]> {
  if (Array.isArray(products)) {
    if (products.length === 0) return products;
    const ids = products.map(p => p._id);
    const variants = await Variant.find({ productId: { $in: ids } }).lean();
    for (const p of products) {
      (p as any).variants = variants.filter(v => v.productId.toString() === p._id.toString());
    }
    return products;
  } else if (products) {
...
export const findPublic = async (query: Query, skip: number, limit: number, sortOrder?: Record<string, any>) => {
  const products = await Product.find(query)
    .populate("categoryId",  CATEGORY_FIELDS)
    .populate("categoryIds", CATEGORY_FIELDS)
    .populate("brandId",     BRAND_FIELDS)
    .sort(sortOrder ?? { createdAt: -1 }).skip(skip).limit(limit).lean();
  return attachVariants(products);
};

/** Fetch products by specific IDs, preserving the supplied order (for price-sorted results) */
export const findPublicByIds = async (ids: mongoose.Types.ObjectId[]): Promise<ProductDocument[]> => {
  if (ids.length === 0) return [];
  const products = await Product.find({ _id: { $in: ids } })
    .populate("categoryId",  CATEGORY_FIELDS)
    .populate("categoryIds", CATEGORY_FIELDS)
    .populate("brandId",     BRAND_FIELDS).lean();
  // Re-sort to maintain aggregate order
  const productMap = new Map(products.map(p => [p._id.toString(), p]));
  const sorted = ids.map(id => productMap.get(id.toString())).filter(Boolean) as ProductDocument[];
  return attachVariants(sorted);
};

export const findAdmin = async (query: Query, skip: number, limit: number) => {
  const products = await Product.find(query)
...
export const save = (product: ProductDocument) =>
  product.save();
...
export const findCategoryIdsWithDescendants = async (slug: string) => {
  const root = await Category.findOne({ slug, isActive: true }).select("_id").lean();
  if (!root) return [];
  ...
  while (currentIds.length > 0) {
    const children = await Category.find({ parentId: { $in: currentIds }, isActive: true }).select("_id").lean();
