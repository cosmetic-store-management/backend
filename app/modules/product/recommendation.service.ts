...
export const getRecommendations = async (userId: string | null, limit = 10) => {
    ...
    if (user) {
      ...
      if (recentProductIds.length > 0) {
        ...
        if (uniqueCategoryIds.length > 0) {
          const recommendedProducts = await productRepo.findPublic(
            {
  ...
  const topSelling = await productRepo.findPublic(
    { isActive: true },
    0,
    limit,
    { sold: -1 }
