...
router.get("/:slug", catchAsync(async (req, res) => {
  const product = await productService.getPublicProductDetail(req.params.slug as string);
...
router.get("/:id/recommendations", catchAsync(async (req, res) => {
  ...
  const products = await productService.getRecommendedProducts(req.params.id as string, limit);
...
router.get("/admin/list", authenticate, isStaff, catchAsync(async (req, res) => {
  const result = await productService.getAdminProducts({ ...req.query as any, shopId: req.shopId });
  return response.success(res, result);
}));

router.get("/admin/:id", authenticate, isStaff, catchAsync(async (req, res) => {
  const product = await productService.getAdminProductDetail(req.params.id as string);
  return response.success(res, { product });
}));
...
router.patch("/admin/:id/status", authenticate, isManager, validate(UpdateProductStatusSchema), catchAsync(async (req, res) => {
  const product = await productService.updateProductStatus(req.params.id as string, req.body.isActive, req.shopId);
...
router.patch("/admin/:id", authenticate, isManager, validate(UpdateProductSchema), catchAsync(async (req, res) => {
  const product = await productService.updateProduct(req.params.id as string, { ...req.body, shopId: req.shopId });
...
router.delete("/admin/:id", authenticate, isManager, catchAsync(async (req, res) => {
  const product = await productService.getAdminProductDetail(req.params.id as string, req.shopId);
