...
import { UpdateOrderStatusInput, CreateOrderInput } from "./dto/order.request.dto.js";
import mongoose from "mongoose";
import { decrementVoucherUsage, validateVoucher, incrementVoucherUsage } from "../voucher/voucher.service.js";
...
export const restoreStock = async (items: any[]): Promise<void> => {
...
export const getOrdersForAdmin = async ({ orderStatus, channel, userId, search, page = 1, limit = 20, paymentStatus, dateFrom, dateTo, shopId }: AdminOrderQuery) => {
...
export const getOrder = async (orderId: string, requestUser: UserDocument) => {
  ...
  if (!isAdmin && !isOwner) throw forbidden("Bạn không có quyền truy cập đơn hàng này");
...
export const updateOrderStatus = async (orderId: string, data: UpdateOrderStatusInput & { receiverName?: string; phone?: string; trackingCode?: string }, requestUser: UserDocument, shopId?: string | null) => {
  const query: any = { _id: orderId };
  if (shopId) query.shopId = new mongoose.Types.ObjectId(shopId);

  const order = await orderRepo.findOne(query);
  if (!order) throw notFound("Không tìm thấy đơn hàng hoặc đơn hàng không thuộc hệ thống của bạn");
...
export const cancelOrder = async (orderId: string, requestUser: UserDocument) => {
  const order = await orderRepo.findOrderById(orderId);
  if (!order) throw notFound("Không tìm thấy đơn hàng");

  const isAdmin = ["owner", "manager", "staff"].includes(requestUser.role);
  const isOwner = String(order.userId) === String(requestUser._id);
  if (!isAdmin && !isOwner) throw forbidden("Bạn không có quyền thao tác đơn hàng này");

  if (order.orderStatus !== "pending") throw badRequest("Chỉ có thể hủy đơn hàng đang chờ xử lý");
