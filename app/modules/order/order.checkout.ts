...
export const getUserTotalSpent = async (userId: mongoose.Types.ObjectId | string): Promise<number> => {
...
export const previewOrder = async (user: UserDocument | null, data: any) => {
  let subtotal = 0;
  let totalWeight = 0;
  const previewItems: { variantId: string; productId: string; unitPrice: number; quantity: number; lineTotal: number }[] = [];

  for (const item of data.items) {
    if (!mongoose.Types.ObjectId.isValid(item.productId)) throw badRequest("productId không hợp lệ");
    if (!item.variantId || !mongoose.Types.ObjectId.isValid(item.variantId)) throw badRequest("variantId không hợp lệ");

    const variant = await orderRepo.findVariantById(item.variantId);
    if (!variant || variant.productId.toString() !== item.productId) throw notFound("Phân loại hàng không hợp lệ");
    if (!variant.isActive) throw badRequest(`Phân loại ${variant.name} hiện không khả dụng`);
...
export const createOrder = async (user: UserDocument, data: CreateOrderInput & { idempotencyKey?: string }) => {
  if (!data.items || data.items.length === 0) throw badRequest("Giỏ hàng rỗng");
  ...
  if (data.idempotencyKey) {
    ...
    if (existingOrder) {
      ...
      const mapped = mapOrder(existingOrder, (existingOrder as any).items || []);
  ...
  for (const item of data.items) {
    ...
    if (!product.isActive) throw badRequest(`Sản phẩm ${product.name} hiện không khả dụng`);
    
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if ((product.categoryId as any)?.isActive === false) throw badRequest(`Danh mục của sản phẩm ${product.name} hiện không khả dụng`);

    const variant = await orderRepo.findVariantById(item.variantId);
    if (!variant || variant.productId.toString() !== item.productId) throw notFound("Phân loại hàng không hợp lệ");
    if (!variant.isActive) throw badRequest(`Phân loại ${variant.name} của sản phẩm ${product.name} hiện không khả dụng`);
    if (variant.stock < item.quantity) throw badRequest(`Sản phẩm ${product.name} (${variant.name}) không đủ tồn kho`);
  ...
  const totalWeight = normalizedItems.reduce((sum, item: any) => sum + ((item as any).weight || DEFAULT_ITEM_WEIGHT_G) * item.quantity, 0);
  ...
  try {
    newOrder = await orderRepo.createOrder({
      ...
      items: normalizedItems.map(i => { const { weight, ...rest } = i as any; return rest; }),
    });
